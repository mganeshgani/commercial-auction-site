...
const RegistrationLinkGenerator: React.FC<RegistrationLinkGeneratorProps> = ({ onFormConfigChange }) => {
  ...
  const checkFormConfig = async () => {
    try {
      ...
      const configExists = response.data && response.data.fields && response.data.fields.length > 0;
      setHasFormConfig(configExists);
      onFormConfigChange?.(configExists);
      
      // Auto-generate link if config exists
      if (configExists) {
        generateLink();
      }
    } catch (error) {
  ...
  // Don't render anything if no form config exists
